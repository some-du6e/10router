...
describe("DevinCliExecutor ACP session/new", () => {
  ...
  it("exposes body.tools as an MCP server (sets XDG_CONFIG_HOME + writes script)", async () => {
    ...
    // Outbound wire identifier: the MCP handshake advertises the rebranded client name.
    const initialize = child.writes
      .map((w) => { try { return JSON.parse(w.trim()); } catch { return null; } })
      .find((m) => m?.method === "initialize");
    expect(initialize?.params?.clientInfo?.name).toBe("10router");
  });
});
