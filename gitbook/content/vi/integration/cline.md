...
Tích hợp 10router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của 10router.
...
Đặt base URL tới endpoint 10router:
...
**Cho instance 10router từ xa:**
```
https://your-10router-host
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint 10router
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key 10router của bạn
2. Bạn có thể tìm API key trong 10router dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-10router-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình 10router
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng 10router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua 10router
...
3. Nhận giải thích AI qua 10router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng 10router để hiểu context và thực hiện thay đổi
...
2. Xác minh 10router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs 10router để xem lỗi

## Cấu hình Nâng cao

### Dùng Endpoint Từ xa

Để dùng instance 10router bạn tự host trên máy khác (VPS, máy chủ tại nhà, tunnel) thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://your-10router-host`
2. Đảm bảo bạn đã tạo API key trong dashboard của instance đó
3. Đảm bảo instance đang chạy và truy cập được từ máy của bạn
...
## Tích hợp với Tính năng 10router

### Định tuyến Model

10router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, 10router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua 10router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
