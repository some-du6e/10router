...
10routerをCline VSCode拡張機能と統合し、AIリクエストを10routerのインテリジェントルーティングシステム経由でルーティングします。
...
**リモートの10routerインスタンスの場合:**
```
https://your-10router-host
```

**手順:**
1. **Base URL** フィールドに10routerエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
3. 10router経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. Clineは10routerを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
2. 10routerインスタンスが動作中で正常か確認
3. VSCodeウィンドウをリロードしてみる (Cmd/Ctrl + Shift + P → 「Reload Window」)
4. エラーについて10routerログを確認

## 高度な設定

### リモートエンドポイントを使用

localhostの代わりに、別のマシン(VPS、自宅サーバー、トンネル)でセルフホストしている10routerを使用:

1. Cline設定で、Base URLを設定: `https://your-10router-host`
2. そのインスタンスのダッシュボードでAPIキーを作成済みか確認
3. インスタンスが起動していて、手元のマシンから到達できるか確認
...
10routerは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、10routerは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

10routerダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
