...
リモートアクセスと本番利用のため、VPSまたはDockerに10routerをデプロイ。
...
| `DATA_DIR` | `~/.9router` | データベースとデータの保存パス （ディレクトリ名は後方互換性のため以前のリリースから変更されていません） |
...
`/etc/nginx/sites-available/10router` を作成:
...
# リバースプロキシを使用しない場合、10routerポートを許可
