...
10routerは、インテリジェントなルーティングと自動フォールバックを通じて、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。

---

## 10routerとは?

10routerは、コーディングツール(Cursor、Cline、Claude Desktop)とAIプロバイダーの間に位置するスマートプロキシです。クォータ、コスト、可用性に基づいて、リクエストを最適なモデルへ自動的にルーティングします。
...
- VPSやDockerで自分のインスタンスをデプロイ([クラウドデプロイ](deployment/cloud.md)を参照)
- iPad、スマホ、どこからでもアクセス
- localhost制限なし
- 自分のドメインでインターネットから到達可能
