...
**10routerは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、10router自体は100%無料でオープンソースです。**
...
**10routerはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## 10routerはCursorで動作しますか?

**はい、ただしCursorは公開アクセス可能なエンドポイントが必要です。**

**問題:** Cursor IDEはlocalhostエンドポイントをサポートしていません。

**解決策:** 10routerを公開URLでホストし、Cursorをそこに向けます:
...
## 10routerをセルフホストできますか?

**はい! 10routerは複数のデプロイメントオプションをサポートします:**
...
**はい、10routerはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.9router`(またはカスタム`DATA_DIR`)にローカル保存 （ディレクトリ名は後方互換性のため以前のリリースから変更されていません）
- 10routerサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/some-du6e/10router/blob/main/CHANGELOG.md)を確認
...
詳細は[CONTRIBUTING.md](https://github.com/some-du6e/10router/blob/main/CONTRIBUTING.md)を参照。
