...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that 10router tries in order.
...
10router automatically tries each model in sequence until one succeeds.
...
When limit reached, 10router skips paid models and uses free tier only.
