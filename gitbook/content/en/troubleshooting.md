...
Common issues and solutions when using 10router.
...
1. **Auto-refresh (default):**
   10router automatically refreshes tokens. Wait 30 seconds and retry.
...
4. **Use a public endpoint:**
   If localhost doesn't work (e.g., Cursor IDE), expose your own instance (VPS or tunnel) and use its URL:
   ```
...
1. **Check if 10router is running:**
   ```bash
   # Check process
